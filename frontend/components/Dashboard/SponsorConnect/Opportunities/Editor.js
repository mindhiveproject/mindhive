...
export default function OpportunityEditor({ opportunityId, user }) {
  ...
  const handleCancel = () => {
    ...
    router.replace({ pathname: "/dashboard/sponsor-connect/opportunities" });
  };

  const notifySponsor = async ({ titleKey, messageKey, defaultTitle, defaultMessage }) => {
    const mentorId = opportunity?.mentor?.id;
    if (!mentorId) return;
    const link = `/dashboard/sponsor-connect/opportunities?op=${opportunityId}`;
    try {
      await sendEmail({
        receiverId: mentorId,
  ...
  const primaryBusy = saving || followUpSaving;
  ...
  const saveDraftLabel = primaryBusy
    ? t("opportunityEditor.saving", {}, { default: "Saving…" })
    : t("opportunityEditor.saveDraft", {}, { default: "Save draft" });
  const submitForReviewLabel = primaryBusy
    ? t("opportunityEditor.saving", {}, { default: "Saving…" })
            ...
            {isReviewMode ? (
              <>
                {canSaveFollowUp && (
                  <Button
                    type="button"
                    variant={reviewPrimaryAction ? "outline" : "filled"}
