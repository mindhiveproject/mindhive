...
export function OrganizationHero({ organization, actions = null }) {
...
export function OrganizationAboutSection({ organization }) {
  const { t } = useTranslation("connect");
  const org = organization;
  ...
  return (
    <>
      ...
      {org.interests?.length > 0 ? (
        <Card>
          <h2>
            {t("organizationsDetail.interests", {}, { default: "Interests" })}
          </h2>
          <InterestChips>
...
export function OrganizationOpportunitiesSection({ organization }) {
  const { t } = useTranslation("connect");
  const org = organization;
  ...
  return (
    <Card>
      ...
      {org.opportunities?.length > 0 ? (
        ...
        <p className="helper">
          {t(
            "organizationsDetail.noOpportunities",
            { name: org.name },
            {
